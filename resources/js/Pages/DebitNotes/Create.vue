...
const props = defineProps({
    suppliers: Array,
    prefillPurchase: Object,
});

const prefill = props.prefillPurchase;

const form = useForm({
    supplier_id:         prefill?.supplier_id || '',
    purchase_invoice_id: prefill?.id || '',
    debit_note_date:     new Date().toISOString().slice(0, 10),
    supply_type:         prefill?.supply_type || 'intrastate',
    reason:              '',
    notes:               '',
...
function submit() { form.post(route('debit-notes.store')); }
</script>
...
<template>
    ...
    <AuthenticatedLayout>
        <template #header>
            <h2 class="text-xl font-semibold text-gray-800">New Debit Note (Purchase Return)</h2>
        </template>
            ...
            <div class="mx-auto max-w-7xl sm:px-6 lg:px-8">
                <form @submit.prevent="submit" class="space-y-6">
                    ...
                    <div class="bg-white rounded-lg shadow p-6">
                        <h3 class="text-base font-semibold text-gray-800 mb-4">Debit Note Details</h3>

                        <!-- Prefill Banner -->
                        <div v-if="prefillPurchase" class="mb-4 px-4 py-3 bg-yellow-50 border border-yellow-200 rounded-lg text-sm text-yellow-800">
                            Pre-filled from Purchase Bill <strong>{{ prefillPurchase.bill_number }}</strong>. Adjust quantities/amounts for the returned items.
                        </div>
                        ...
                        <div class="grid grid-cols-3 gap-4">
                            ...
                            <div>
                                ...
                                <input v-model="form.debit_note_date" type="date" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm" required />
                            </div>
                            <div>
                            ...
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Against Purchase Bill</label>
                                <input v-model="form.purchase_invoice_id" type="text" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm" placeholder="Optional bill ID" />
                            </div>
                            <div class="col-span-2">
                                <label class="block text-sm font-medium text-gray-700 mb-1">Return Reason</label>
                                <input v-model="form.reason" type="text" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm" placeholder="e.g. Defective goods, Wrong items received..." />
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bg-white rounded-lg shadow p-6">
                        <div class="flex justify-between items-center mb-4">
                            <h3 class="text-base font-semibold text-gray-800">Returned Items</h3>
                        ...
                        <div class="flex justify-end mt-4">
                            <div class="w-72 space-y-1 text-sm">
                                ...
                                <div class="flex justify-between text-red-600"><span>Tax on Return</span><span>{{ fmt(totals.tax) }}</span></div>
                                <div class="flex justify-between font-bold text-gray-900 border-t pt-1"><span>Total Return Value</span><span>{{ fmt(totals.total) }}</span></div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="flex justify-end gap-3">
                        <a :href="route('debit-notes.index')" class="px-4 py-2 border border-gray-300 rounded-lg text-sm text-gray-700 hover:bg-gray-50">Cancel</a>
                        <button type="submit" :disabled="form.processing" class="px-6 py-2 bg-indigo-600 text-white rounded-lg text-sm font-medium hover:bg-indigo-700 disabled:opacity-50">
                            {{ form.processing ? 'Creating...' : 'Create Debit Note' }}
                        </button>
                    </div>
