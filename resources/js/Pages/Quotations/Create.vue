...
function onProductSelect(i) {
    ...
    if (p) {
        ...
        form.items[i].price = p.price;
        form.items[i].gst_rate = p.gst_rate;
        form.items[i].unit = p.unit;
        form.items[i].hsn_sac_code = p.hsn_sac_code || '';
    }
}
...
function submit() { form.post(route('quotations.store')); }
</script>
...
<template>
    ...
    <AuthenticatedLayout>
        <template #header>
            <h2 class="text-xl font-semibold text-gray-800">Create Quotation / Proforma Invoice</h2>
        </template>
            ...
            <div class="mx-auto max-w-7xl sm:px-6 lg:px-8">
                <form @submit.prevent="submit" class="space-y-6">
                    ...
                    <div class="bg-white rounded-lg shadow p-6">
                        <h3 class="text-base font-semibold text-gray-800 mb-4">Quotation Details</h3>
                        <div class="grid grid-cols-2 gap-4">
                            ...
                            <div>
                                ...
                                <input v-model="form.quotation_date" type="date" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm" required />
                            </div>
                            <div>
                            ...
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Quote Type</label>
                    ...
                    <div class="bg-white rounded-lg shadow p-6">
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <textarea v-model="form.notes" rows="3" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm" placeholder="Additional notes..."></textarea>
                            </div>
                            <div>
                                <label class="block text-sm font-medium text-gray-700 mb-1">Terms & Conditions</label>
                                <textarea v-model="form.terms" rows="3" class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm" placeholder="Payment terms, validity, delivery..."></textarea>
                            </div>
                        </div>
                    </div>

                    <div class="flex justify-end gap-3">
                        <a :href="route('quotations.index')" class="px-4 py-2 border border-gray-300 rounded-lg text-sm text-gray-700 hover:bg-gray-50">Cancel</a>
